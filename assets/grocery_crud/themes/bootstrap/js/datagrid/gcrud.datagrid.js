...
(function ($) {
    ...
    Datagrid.prototype.hideShowDeleteButton = function () {
        ...
        if (this.gcrud_container.find('.select-row:checked').length > 0) {
            ...
            this.gcrud_container.find('.renta-selected-button').removeClass('hidden');
        } else {
            this.gcrud_container.find('.delete-selected-button').addClass('hidden');
            this.gcrud_container.find('.renta-selected-button').addClass('hidden');
        }
    };
    ...
    Datagrid.prototype.listenerDeleteMultiple = function () {
        ...
        this.gcrud_container.find('.renta-selected-button').click(function () {

            this.gcrud_container.find('.renta-multiple-confirmation').gc_modal();
            this.gcrud_container.find('.renta-multiple-confirmation').on('hidden.bs.modal', function () {
                this.gcrud_container.find('.renta-multiple-confirmation-button').unbind('click');
            }.bind(this));

            this.gcrud_container.find('.renta-multiple-confirmation-button').click(function (event) {
                event.preventDefault();

                var my_modal = this.gcrud_container.find('.renta-multiple-confirmation:first'),
                    delete_selected = [],
                    data_to_send,
                    urlPath = this.gcrud_container.find('.renta-multiple-confirmation-button').data('target');
                ...
                this.sendAjaxPost(data_to_send, urlPath, function (output) {
                    if (output.success) {
                        $.growl('Se cambió el estatus de los productos seleccionados', {
                            type: 'success',
                            delay: 10000,
                            animate: {
                                enter: 'animated bounceInDown',
